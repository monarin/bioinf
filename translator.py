...
class InvalidTranscriptName(TranslatorError):
    pass


class InvalidTranscriptCoordinate(TranslatorError):
    pass


class InvalidChromosomeName(TranslatorError):
    pass


class InvalidChromosomeCoordinate(TranslatorError):
    pass
...
def createTranslateTbl(CIGARObj: Dict, mode: str ="TR2CHR") -> Dict:
    ...
    for TRName, TRVal in CIGARObj.items():
        for CHRName, CHRVal in TRVal.items():
            ...
            try:
                CIGARValTR = [
                    int(pair[0]) * int(pair[1] in ("M", "I")) for pair in CIGARMatch
                ] + [0]
                CIGARValCHR = [
                    int(pair[0]) * int(pair[1] in ("M", "D")) for pair in CIGARMatch
                ] + [0]
                if mode == "TR2CHR":
                    CIGARList = [
                        (sum(CIGARValTR[:i]), sum(CIGARValCHR[:i]) + startingPosition)
                        for i in range(len(CIGARMatch) + 1)
                    ]
                else:
                    CIGARList = [
                        (sum(CIGARValCHR[:i]) + startingPosition, sum(CIGARValTR[:i]))
                        for i in range(len(CIGARMatch) + 1)
                    ]
            except ValueError:
